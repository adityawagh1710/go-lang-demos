...
func TestGetAllCSVFiles_ReturnsOnlyCSV(t *testing.T) {
	dir := t.TempDir()
...
func TestGetAllCSVFiles_EmptyDir(t *testing.T) {
	dir := t.TempDir()
	files, err := GetAllCSVFiles(dir)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
...
func TestGetAllCSVFiles_InvalidDir(t *testing.T) {
	_, err := GetAllCSVFiles("/nonexistent/path/xyz")
	if err == nil {
		t.Error("expected error for invalid directory")
	}
}

func TestGetAllCSVFiles_NoCSVFiles(t *testing.T) {
	dir := t.TempDir()
